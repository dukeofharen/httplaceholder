...
There are two Vagrant boxes (Windows and Ubuntu) that you can use to view how installation of HttPlaceholder is done. You can find them in this repository under the folder "vagrant". You need to have [Vagrant](https://www.vagrantup.com/) installed. Also, make sure you either built HttPlaceholder (run `build.sh` in the root of this repository) or downloaded the files `httplaceholder_linux-x64.tar.gz` and `httplaceholder_win-x64.zip` (see https://github.com/dukeofharen/httplaceholder/releases/latest) After that, it's just a matter of going to the correct folder in your terminal and typing `vagrant up`. HttPlaceholder will then be installed under Windows or Ubuntu and can be reached by going to `http://localhost:8080` or `https://localhost:4430`.
