...
namespace HttPlaceholder.Utilities
{
    public static class ConfigurationUtilities
    {
        ...
        public static IConfigurationBuilder AddConfiguration(this IConfigurationBuilder builder, IDictionary<string, string> argsDictionary)
        {
            var configDictionary = new Dictionary<string, string>();
            foreach (var constant in GetConfigKeyMetadata())
            {
                if (constant.configPath != null && argsDictionary.TryGetValue(constant.configKey, out string value))
                {
