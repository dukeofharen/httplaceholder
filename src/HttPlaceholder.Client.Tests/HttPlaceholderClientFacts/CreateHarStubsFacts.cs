...
[TestClass]
public class CreateHarStubsFacts : BaseClientTest
{
    private const string CreateHarStubsResult = @"[
    {
        ""stub"": {
    ...
    [TestMethod]
    public async Task CreateHarStubs_ExceptionInRequest_ShouldThrowHttPlaceholderClientException()
    {
        // Arrange
        const string har = "HAR JSON";
    ...
    [DataRow(true)]
    [DataRow(false)]
    public async Task CreateHarStubs_ShouldCreateStubs(bool doNotCreateStub)
    {
        // Arrange
        const string har = "HAR JSON";
        var client = new HttPlaceholderClient(CreateHttpClient(mock => mock
