...
// TODO dit naar nieuw formaat omzetten
export const useStubFormStore = defineStore('stubForm', {
  state: () =>
    ({
      input: '',
  ...
  getters: {
    ...
    getCurrentSelectedFormHelper: (state): FormHelperKey => state.currentSelectedFormHelper,
    getFormIsDirty: (state): boolean => state.formIsDirty,
    getFormHelperSelectorFilter: (state): string => state.formHelperSelectorFilter,
    ...
    getUrlPath(state): any {
      return handle((parsed) => parsed?.conditions?.url?.path ?? '', state.input)
    },
    getFullPath(state): any {
      return handle((parsed) => parsed?.conditions?.url?.fullPath ?? '', state.input)
    },
    getQuery(state): any {
      return handle((parsed) => parsed?.conditions?.url?.query ?? '', state.input)
    },
    getRequestHeaders(state): any {
      return handle((parsed) => parsed?.conditions?.headers ?? {}, state.input)
    },
    getClientIp(state): any {
      return handle((parsed) => parsed?.conditions?.clientIp ?? '', state.input)
    },
    ...
    getScenarioMinHits(state): any {
      return handle((parsed) => parsed?.conditions?.scenario?.minHits ?? 0, state.input)
    },
    getScenarioMaxHits(state): any {
      return handle((parsed) => parsed?.conditions?.scenario?.maxHits ?? 0, state.input)
    },
    getScenarioExactHits(state): any {
      return handle((parsed) => parsed?.conditions?.scenario?.exactHits ?? 0, state.input)
    },
    getScenarioStateCheck(state): any {
      return handle((parsed) => parsed?.conditions?.scenario?.scenarioState ?? '', state.input)
    },
    getBasicAuth(state): StubBasicAuthenticationModel | undefined {
      return handle((parsed) => parsed?.conditions?.basicAuthentication, state.input)
    },
    getResponseContentType(state): any {
      return handle((parsed) => parsed?.response?.contentType ?? '', state.input)
    },
    getExtraDuration(state): any {
      return handle((parsed) => parsed?.response?.extraDuration ?? 0, state.input)
    },
  },
  actions: {
    ...
    setResponseContentType(contentType: string): void {
      handle((parsed) => {
        ...
        if (parsed.response.headers) {
          const key: string | undefined = Object.keys(parsed.response.headers).find(
