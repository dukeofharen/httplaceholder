<template>
  <div>
    ...
    <div class="col-md-12 mb-3">
      <router-link class="btn btn-success me-2 btn-mobile full-width" :to="{ name: 'ScenarioForm' }"
        >{{ $translate('scenarios.addScenario') }}
      </router-link>
      <button class="btn btn-danger btn-mobile full-width" @click="clearAllScenariosModal = true">
      ...
      <ul class="list-group">
        ...
        >
          ...
          <div>
            ...
            <button class="btn btn-danger btn-sm" @click="deleteScenario(scenario.scenario)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const initializeSignalR = async () => {
      signalrConnection = new HubConnectionBuilder().withUrl(`${getRootUrl()}/scenarioHub`).build()
      signalrConnection.on('ScenarioSet', (scenario: ScenarioModel) => {
        const foundScenario = scenarios.value.find((s) => s.scenario === scenario.scenario)
        if (foundScenario) {
      ...
      signalrConnection.on('ScenarioDeleted', (scenarioName: string) => {
        const foundScenario = scenarios.value.find((s) => s.scenario === scenarioName)
        if (foundScenario) {
          scenarios.value = scenarios.value.filter((s) => s.scenario !== scenarioName)
        }
      })
    ...
    // Lifecycle
    onMounted(async () => await Promise.all([loadScenarios(), initializeSignalR()]))
    onUnmounted(() => {
