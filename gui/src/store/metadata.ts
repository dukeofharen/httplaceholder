...
export const useMetadataStore = defineStore({
  ...
  actions: {
    checkFeatureIsEnabled: (feature: FeatureFlagType): Promise<FeatureResultModel> =>
      get(`/ph-api/metadata/features/${feature}`),
    async checkAuthenticationIsEnabled(): Promise<boolean> {
      const authEnabled = (await this.checkFeatureIsEnabled(FeatureFlagType.Authentication)).enabled
